...
This installation guide takes [Crypki](https://github.com/theparanoids/crypki) as signing backend. 
...
> **Disclaimer:**  Following guidelines are to help you to get started with PSSHCA; 
...
A valid gensign configuration file can be found [here](config.sample.json);
it can be directly plugged into a configuration file for the SSHRA docker.
...
In order to identify the types and usages of an SSH certificates for different PAM modules, PSSHCA defines an extensible key ID format for user certificates. 
