...
SSHRA utilizes [Go SSH](https://pkg.go.dev/golang.org/x/crypto/ssh) to generate SSH certificates and CSRs, which follow the format defined in [OpenSSH](https://www.openssh.com/specs.html).  
In order to identify the types and usages of an SSH certificates for different PAM modules, SSHRA defines an extensible key ID format for user certificates. 

Usually, the `Key ID` field in a SSH certificate is used to identify the specific private key that may be stored on the HSM. 
However, `Key ID` in SSHRA is in JSON format, which enables gensign and different CSR handlers (or modules) to fill detailed information into CSR and certificates.
